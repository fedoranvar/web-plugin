...
function get_checked_or_all_rows() {
  let all_rows = Array.from(document.querySelectorAll('.main_table.nowrap tbody tr'))
  let checked_rows = all_rows.filter(el => { return el.querySelector("input:checked") })

  return ((checked_rows.length > 0) ? checked_rows : all_rows)
}
...
function batch_get_document_to_save() {
  ...
  var rows =  get_checked_or_all_rows()
...
function batch_get_document_to_sign() {
  var o = [];
  setColumn();
  console.log('batch_get_document_to_sign');
  try {
    var list = [];
    // let rows = document.querySelectorAll('#ItemsForAction')
    let rows = get_checked_or_all_rows()
...
// generateLink('Запустить авто подписание', batch_get_document_to_sign)
...
window.addEventListener("message", function(event) {
  ...
  if (event.source == window && event.data.action == "batch_get_document_to_sign") {
    batch_get_document_to_sign();
  }
